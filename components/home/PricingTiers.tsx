...
export function PricingTiers() {
  return (
    <section
      id="pricing-section"
      className="mx-auto mb-14 max-w-[760px] animate-fadeUp py-12"
    >
      {/* Header */}
      <div className="mb-8 text-center">
        ...
        <p className="text-[13px] leading-[1.7] text-grove-muted2">
          Tidak ada kebingungan tier. Member dapat akses penuh — program
          tambahan dibeli sukarela sesuai kebutuhan.
        </p>
      </div>
      ...
      <div className="mb-5 grid gap-[14px] min-[761px]:grid-cols-[1fr_1.15fr]">
            ...
            {freeFeatures.map((f) => (
              <p
                ...
                className={`text-[11.5px] leading-[1.5] ${
                  f.active ? "text-grove-muted2" : "text-grove-muted"
                }`}
              >
                {f.text}
              </p>
        ...
        <article className="relative rounded-grove-3 border-2 border-grove-primary bg-grove-bg2 p-7">
          ...
          <div className="mb-1 flex items-baseline gap-2">
            <p className="font-serif text-[40px] font-bold leading-none">
              Rp 249K
            </p>
          ...
          <div className="grid gap-x-3 gap-y-1.5 min-[481px]:grid-cols-2">
            <div className="space-y-1.5">
              {memberLeft.map((f) => (
                <p
                  key={f}
                  className="text-[11.5px] leading-[1.5] text-grove-muted2"
                >
                  {f}
                </p>
              ))}
      ...
      <div className="rounded-grove-3 border border-grove-border bg-grove-bg2 p-6">
        ...
        <div className="mb-4 flex items-start justify-between">
          <div>
            <p className="mb-1 text-[10px] font-bold uppercase tracking-[.12em] text-[#F0A030]">
              Add-on · Beli Satuan
            </p>
          ...
          <p className="text-right text-[11px] leading-[1.5] text-grove-muted">
            Member dapat{" "}
            <span className="font-medium text-grove-primary">
              harga lebih murah
            </span>
          </p>
        </div>
          ...
          {addons.map((a) => (
            ...
            >
              <div className="mb-2 flex flex-wrap items-center gap-1">
                <p className={`text-[9.5px] font-medium ${a.tagColor}`}>
                  {a.tag}
                </p>
                {a.tagExtra && (
                  <>
                    <span className="text-[9px] text-grove-muted">·</span>
                    <p className="text-[9.5px] font-medium text-[#c8a84b]">
                      {a.tagExtra}
                    </p>
                  </>
                )}
              ...
              <p className="mb-1 text-[13px] font-bold text-grove-primary">
                {a.price}
              </p>
              <p className="text-[10px] leading-[1.5] text-grove-muted">
                {a.desc}
              </p>
            </div>
          ))}
        ...
        <div className="mb-4 flex flex-wrap gap-x-6 gap-y-1.5 border-t border-grove-border pt-4">
          <p className="text-[11.5px] text-grove-muted2">
            +{" "}
            <span className="font-medium text-[#F0A030]">
              Grove Field Research Trip
            </span>{" "}
            Rp 1.5 jt (non-member Rp 2.5 jt)
          </p>
          <p className="text-[11.5px] text-grove-muted2">
            +{" "}
            <span className="font-medium text-[#9A72E8]">
              Grove Summit Tahunan
            </span>{" "}
            Rp 349K early bird
          </p>
          <p className="text-[11.5px] text-grove-muted2">
      ...
      <p className="text-center text-[11px] text-grove-muted mt-2 pt-0.5">
        Batalkan kapan saja · tidak ada biaya tersembunyi · data tidak dijual ke
        pihak ketiga
      </p>
    </section>
  );
